...
def home(request):
    ...
    print(request)
    city_deleted = request.GET.get('deleted', False)  #were we redirected from the delete page
    print(city_deleted)
    if not city_deleted:  #If not get current weather data and save it.
    ...
    else:   #If so then we will get the saved data
